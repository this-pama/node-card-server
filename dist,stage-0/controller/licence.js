...
exports.default = function (_ref) {
  ...
  api.get('/:serial', function (req, res) {
    _licence2.default.find({ serial: req.params.serial }, function (err, user) {
      if (err) {
        console.log(err);
      }
      res.json(user);
  ...
  api.put('/licence', function (req, res) {
    _licence2.default.find({ serial: req.body.serial }, function (err, user) {
      if (err) {
        console.log(err);
      }
      ...
      try {
        ...
        if (user[0].username === req.body.username) {
          if (user[0].imei === req.body.imei || user[0].phoneSerial === req.body.phoneSerial) {
            ...
            var id = user[0]._id;
            _licence2.default.findByIdAndUpdate(id, update, function (err, user) {
              if (err) {
                console.log(err);
              }
  ...
  api.post('/add', function (req, res) {
    ...
    // create user profile
    var newLicence = new _licence2.default();
