...
import SubCpLicence from '../model/subCpLicence';
...
export default({ config, db }) => {
  ...
  api.get('/:serial', (req, res) => {
    SubCpLicence.find( {serial: req.params.serial} , (err, user) => {
      if (err) {
        console.log(err)
      }
      res.json(user);
  ...
  api.put('/licence', (req, res) => {
    SubCpLicence.find({serial: req.body.serial }, (err, user) => {
      if (err) {
        console.log(err)
      }
        ...
        if(user[0].username === req.body.username){
            if( user[0].imei=== req.body.imei || user[0].phoneSerial ===req.body.phoneSerial  ){
             ...
             let ref = randomize('a', 20);
              ...
              let id = user[0]._id
              SubCpLicence.findByIdAndUpdate( id, update,  (err, user) => {
                if (err){
                  console.log(err)
                }
...
// '/v1/licence/add' - POST - add a serial 
  api.post('/add', (req, res) => {
        ...
        // create user profile
        let newLicence = new SubCpLicence();
