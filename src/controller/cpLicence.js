...
import CpLicence from '../model/cpLicence';
...
export default({ config, db }) => {
  ...
  api.get('/:serial', (req, res) => {
    CpLicence.find( {serial: req.params.serial} , (err, user) => {
      if (err) {
        console.log(err)
      }
      res.json(user);
  ...
  api.put('/licence', (req, res) => {
    CpLicence.find({serial: req.body.serial }, (err, user) => {
      if (err) {
        console.log(err)
      }
        ...
        if(user[0].username === req.body.username){
            if( user[0].imei=== req.body.imei || user[0].phoneSerial ===req.body.phoneSerial  ){
             ...
             let ref = randomize('a', 20);
              ...
              let id = user[0]._id
              CpLicence.findByIdAndUpdate( id, update,  (err, user) => {
                if (err){
                  console.log(err)
                }
